...
public class MoveScanner {
	...
	public static class ScannedChunk implements Comparable<ScannedChunk> {
		...
		public void scanBlocks(Chunk chunk, int chunkY, ScannerPolicy policy) {
			...
			BlockStateContainer array = storage.getData();
			for (int y = 0; y < 16; y++) {
				for (int z = 0; z < 16; z++) {
					int oResult = local(0, y, z);
					//int oFrom = (y << 8 | z << 4);
					for (int x = 0; x < 16; x ++) {
						int data = Block.getIdFromBlock(array.get(x,  y,  z).getBlock());
			...
			ClassInheritanceMultiMap<Entity> entities = chunk.getEntityLists()[chunkY];
